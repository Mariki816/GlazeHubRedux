...
from model_shawn import connect_to_db, db, Chem, User, Recipe, Component
...
#     print "In processLogin"
    email = request.form.get("userEmail").lower()
    pword = request.form.get("password")
    user = User.getUserByEmail(email)

#     user_id_test = user.getUserID()
#     print "This is user_id_test", user_id_test


    if user:
        pwordcheck = user.getUserPasswordByEmail(email)
        if pword == pwordcheck:
...
def getNewUser():
    ...
    if User.getUserByEmail(newUser.email):
        flash("Email address already exists. Please log on or enter new email")
        return render_template("login.html")
    else:
        db.session.add(newUser)
        db.session.commit()
# 	print "This is newUser.id ", int(newUser.id)
# 	print "this is newUser.id type ", type(newUser.id)
    newUser_id = int(newUser.id)
    flash("Welcome, %s" % (newUser.user_name))
    do_login(newUser_id, newUser.email, newUser.user_name)
    return redirect("/userRecipes/%d" % newUser_id)
...
def renderEnterRecipeForm():
    ...
    chems = db.session.query(Chem).all()
...
def enterRecipe():
    ...
    for chem in chem_list:
        comp = Component()
        comp.chem_name = chem
        comp.chem_id = Chem.getChemIDByName(comp.chem_name)
...
def addRecipe(userViewID):
    ...
    components = Component.getComponentsByRecipeID(newRecipe.id)
...
def recipe(userViewID, recipeName):
    ...
    if checkUserLoginID(userLoginID, userViewID) is False:
        userViewID = userLoginID
        recipes = Recipe.getRecipeNamesByUserID(userLoginID)
        return render_template("user_recipes.html", displayRecipes=recipes,
                               user_id=userLoginID)
    else:
        recipe = Recipe.getRecipeIDByName(recipeName, userViewID)
        components = Component.getComponentsByRecipeID(recipe.id)
...
def batchSizeChange(userViewID, recipeName):
    ...
    for i in range(len(batchComp)):
        ...
        batchComp[i] = (sizeflt * batchComp[i])*newPercent
        chemID = Chem.getChemIDByName(components[j].chem.chem_name)
...
def showUserRecipeList(userViewID):
    ...
    if checkUserLoginID(userLoginID, userViewID):
        ...
        recipes = Recipe.getRecipeNamesByUserID(userViewID)
        return recipes
